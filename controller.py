"""Software to monitor and control Lights via a raspberry pi."""
...
class Light(object):
    ...
    def __init__(self, LightId, config):
        self.id = LightId
...
class Controller(object):
    def __init__(self, config):
        ...
        self.config = config
        self.Lights = [Light(n, c) for (n, c) in sorted(config['lights'].items())]
        self.updateHandler = UpdateHandler(self)
        for Light in self.Lights:
            Light.last_state = 'unknown'
            Light.last_state_time = time.time()
    ...
    def status_check(self):
        for Light in self.Lights:
            new_state = Light.get_state()
            if (Light.last_state != new_state):
                syslog.syslog('%s: %s => %s' % (Light.name, Light.last_state, new_state))
                Light.last_state = new_state
                Light.last_state_time = time.time()
                self.updateHandler.handle_updates()
                if self.config['config']['use_openhab'] and (new_state == "on" or new_state == "off"):
                    self.update_openhab(Light.name, new_state)
                if self.config['config']['use_ifttt'] and (new_state == "on" or new_state == "of"):
                    self.update_ifttt(Light.name, new_state, Light.ifttt_event_open, Light.ifttt_event_close)
            if new_state == 'on' and not Light.msg_sent and time.time() - Light.open_time >= self.ttw:
                if self.use_alerts:
                    title = "%s's light on" % Light.name
                    if self.ttw == 0:
                        message = "%s's light just get on" % (Light.name)
                    else:
                        etime = elapsed_time(int(time.time() - Light.open_time))
                        message = "%s's Light has been open for %s" % (Light.name, etime)
                    self.send_msg(Light, title, message)
                    Light.msg_sent = True

            if new_state == 'off':
                if self.use_alerts:
                    if Light.msg_sent == True:
                        title = "%s's light off" % Light.name
                        etime = elapsed_time(int(time.time() - Light.open_time))
                        message = "%s's light is now off after %s "% (Light.name, etime)
                        self.send_msg(Light, title, message)
                Light.open_time = time.time()
                Light.msg_sent = False

    def send_msg(self, Light, title, message):
        for alert in self.alert_type:
            if alert == 'smtp':
                self.send_email(title, message)
            elif alert == 'pushbullet':
                self.send_pushbullet(Light, title, message)
            elif alert == 'pushover':
                self.send_pushover(Light, title, message)
            elif alert == 'telegram':
                self.send_telegram(Light, title, message)
    ...
    def send_pushbullet(self, Light, title, message):
        try:
            syslog.syslog("Sending pushbutton message")
            config = self.config['alerts']['pushbullet']

            if Light.pb_iden != None:
                conn = python_http_client.HTTPSConnection("api.pushbullet.com:443")
                conn.request("DELETE", '/v2/pushes/' + Light.pb_iden, "",
            ...
            print(response)
            Light.pb_iden = json.loads(response)['iden']
        except Exception as inst:
            syslog.syslog("Error sending to pushbullet: " + str(inst))

    def send_pushover(self, Light, title, message):
        try:
            syslog.syslog("Sending Pushover message")
            config = self.config['alerts']['pushover']
            conn = python_http_client.HTTPSConnection("api.pushover.net:443")
    ...
    def send_telegram(self, Light, title, message):
        try:
            syslog.syslog("Sending Telegram message")
            config = self.config['alerts']['telegram']
            conn = python_http_client.HTTPSConnection("api.telegram.org:443")
    ...
    def update_ifttt(self, Light, status, open_event, close_event):
        try:
            syslog.syslog("Sending ifttt event")
            config = self.config['ifttt']
            conn = python_http_client.HTTPSConnection("maker.ifttt.com:443")
            if status == "on":
    ...
    def update_openhab(self, item, state):
        try:
            ...
            conn = python_http_client.HTTPConnection("%s:%s" % (config['server'], config['port']))
    ...
    def run(self):
        ...
        if self.config['config']['use_auth']:
            ...
            credentialFactory = BasicCredentialFactory("Light Controller")
...
class ClickHandler(Resource):
    ...
    def render(self, request):
        Light = request.args['id'][0]
        self.controller.toggle(Light)
        return 'OK'
...
class CloseHandler(Resource):
    ...
    def __init__ (self, controller):
        Resource.__init__(self)
        self.controller = controller
        self.Lights = [Light(n, c) for (n, c) in sorted(controller.config['lights'].items())]
...
class APIHandler(Resource):
    ...
    def render(self, request):
        ...
        LightId = request.args['id'][0]
        if key == self.controller.config['config']['api_key']:
            for d in self.Lights:
                if d.id == LightId or d.id == "all_lights":
                    state = d.get_state()
                    if command == "toggle":
...
class StatusHandler(Resource):
    ...
    def render(self, request):
        Light = request.args['id'][0]
        for d in self.controller.Lights:
            if (d.id == Light):
                return d.last_state
        return ''
